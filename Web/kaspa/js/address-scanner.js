...
export class AddressScanner {
    ...
    /**
     * Check balance for a single address using UTXOs
    ...
    async checkAddressBalance(address, networkType, reuseConnection = false) {
        try {
            ...
            if (balanceResponse && balanceResponse.entries && balanceResponse.entries.length > 0) {
                // Address has balance, get the balance amount
                for (const entry of balanceResponse.entries) {
                    if (entry && entry.balance) {
                        totalBalance = BigInt(entry.balance);
                    }
                }
                
                // Now get UTXOs for additional info
            ...
            const balanceKas = parseFloat(sompiToKas(totalBalance));

            return {
                success: true,
                balance: balanceKas,
                balanceSompi: totalBalance,
    ...
    async fetchUTXOsForAddress(address, networkType) {
        try {
            ...
            // Check balances first, then get UTXOs
            const balanceResponse = await this.rpc.getBalancesByAddresses([address]);
 ...
 */
export async function calculateBalanceFromUTXOs(utxos) {
    ...
    const { sompiToKas } = await import('./currency-utils.js');
    const totalBalance = sompiToKas(totalBalanceSompi);
 ...
 */
export function validateUTXOsSufficient(utxos, requiredAmountSompi, requiredFeeSompi = 0n) {
    if (!utxos || utxos.length === 0) {
        return {
            sufficient: false,
            availableBalance: 0n,
