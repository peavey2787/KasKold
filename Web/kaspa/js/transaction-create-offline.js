...
// Calculate balance from cached UTXOs
async function calculateCachedBalance(cachedUTXOs) {
    ...
    const totalBalanceSompi = cachedUTXOs.utxos.reduce((sum, utxo) => {
        const amount = BigInt(utxo.amount || 0);
        return sum + amount;
    }, 0n);

    const { sompiToKas } = await import('./currency-utils.js');
    const totalBalance = sompiToKas(totalBalanceSompi); // Convert to KAS using safe conversion

    return {
        totalBalance: parseFloat(totalBalance),
        totalBalanceSompi: totalBalanceSompi,
        utxoCount: cachedUTXOs.utxos.length
    };
}
